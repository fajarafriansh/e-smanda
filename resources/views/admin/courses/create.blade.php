    ...
    <div class="card-body">
        <form action="{{ route("admin.courses.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('teachers') ? 'has-error' : '' }}">
                <label for="teacher">{{ trans('cruds.course.fields.teacher') }}
                    <span class="btn btn-info btn-xs select-all">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all">{{ trans('global.deselect_all') }}</span></label>
                <select name="teachers[]" id="teachers" class="form-control select2" multiple="multiple">
                    @foreach($teachers as $id => $teacher)
                        <option value="{{ $id }}" {{ (in_array($id, old('teachers', [])) || isset($course) && $course->teachers->contains($id)) ? 'selected' : '' }}>{{ $teacher }}</option>
                    @endforeach
                </select>
...
@if(isset($course) && $course->course_image)
      ...
      this.options.thumbnail.call(this, file, '{{ $course->course_image->getUrl('thumb') }}')
