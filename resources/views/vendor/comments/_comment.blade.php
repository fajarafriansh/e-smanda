...
@if(isset($reply) && $reply === true)
  <div id="comment-{{ $comment->id }}" class="media reply">
@else
...
@endif
    ...
    <div class="avatar mr-3">
        <img class="mr-3" src="{{ asset('img/avatar/'. $comment->commenter->detail->avatar ?? 'default-avatar.png') }}" alt="{{ $comment->commenter->name ?? $comment->guest_name }} Avatar">
    </div>
    <div class="media-body">
        ...
        <div>
            @can('reply-to-comment', $comment)
                <button data-toggle="modal" data-target="#reply-modal-{{ $comment->id }}" class="btn btn-sm btn-link comment-btn">Balas</button>
            @endcan
            @can('edit-comment', $comment)
                <button data-toggle="modal" data-target="#comment-modal-{{ $comment->id }}" class="btn btn-sm btn-link comment-btn">Edit</button>
            @endcan
            @can('delete-comment', $comment)
                <a href="{{ route('comments.destroy', $comment->id) }}" onclick="event.preventDefault();document.getElementById('comment-delete-form-{{ $comment->id }}').submit();" class="btn btn-sm btn-link text-danger">Delete</a>
        ...
        @can('edit-comment', $comment)
            <div class="modal comment fade" id="comment-modal-{{ $comment->id }}" tabindex="-1" role="dialog">
                        ...
                        <form method="POST" action="{{ route('comments.update', $comment->id) }}">
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    <label for="message">Update pesan di sini:</label>
        ...
        @can('reply-to-comment', $comment)
            <div class="modal comment fade" id="reply-modal-{{ $comment->id }}" tabindex="-1" role="dialog">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
